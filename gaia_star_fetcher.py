...
class GaiaStarFetcher:
    ...
    def fetch_nearby_stars(self, max_stars=1000, max_distance_pc=50):
        ...
        -----------
        max_stars : int
            Maximum number of stars to fetch (up to 1 million)
        max_distance_pc : float
            Maximum distance in parsecs (up to 1 million)
        ...
        # For very large queries, add a warning
        if max_stars > 10000:
            print("Note: Large queries may take several minutes to complete...")
...
def main():
    ...
    if df is not None:
        ...
        print("\nNote: For queries of millions of stars, use the Streamlit interface for better progress tracking.")
